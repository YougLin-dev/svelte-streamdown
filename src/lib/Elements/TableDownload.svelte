<script lang="ts">
	...
	const formats: TableControlsFormat[] = ['Markdown', 'HTML', 'CSV'];
	...
	const getContent = (format: TableControlsFormat) => {
		if (format === 'Markdown') return token.raw;
		if (format === 'HTML') return getHtmlContent();
		return getCsvContent();
	};

	const download = (format: TableControlsFormat) => {
		...
		if (format === 'Markdown') {
			save('table.md', content, 'text/markdown');
		} else if (format === 'HTML') {
			save('table.html', content, 'text/html');
		} else {
			save('table.csv', content, 'text/csv');
		}

		popover.isOpen = false;
	};

	const copyContent = async (format: TableControlsFormat) => {
		const content = getContent(format);
		if (!content) return;
	...
	const methods: TableControlsMethods = {
		copy: copyContent,
		download,
		getContent
	};
...
{#if streamdown.controls.table || streamdown.snippets.tableControls}
	<div data-streamdown-table-download class="ml-auto flex w-fit items-center justify-end gap-2 p-1">
		<Slot
			props={{
				token,
